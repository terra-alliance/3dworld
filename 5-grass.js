...
const backgroundMaterial = new THREE.ShaderMaterial({
	uniforms: {
		...
		resolution: {type: 'vec2', value: new THREE.Vector2(grassCanvas.width, grassCanvas.height)},
...
// var skygui = new dat.GUI();
// skygui.add(this, 'radius').min(85).max(1000).step(5);
// skygui.add(this, 'speed').min(0.5).max(10).step(0.01);
// skygui.add(this, 'elevation').min(0.0).max(Math.PI/2.0).step(0.01).listen().onChange(function(value){updateSunPosition();});
// skygui.add(this, 'azimuth').min(0.0).max(Math.PI*2.0).step(0.01).listen().onChange(function(value){updateSunPosition();});
// skygui.add(this, 'fogFade').min(0.001).max(0.01).step(0.0001).listen().onChange(function(value){backgroundMaterial.uniforms.fogFade.value = fogFade;});
// skygui.close();
